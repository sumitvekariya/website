    ...
    <div className="fixed inset-0 z-50" data-command-menu-open>
      ...
      >
        ...
        >
          <div
            id={titleId}
            className="border-b border-foreground/10 px-3 py-2.5 text-[0.75rem] leading-snug text-foreground/70"
          >
            Jump to —{" "}
          ...
          <ul className="max-h-[min(52vh,24rem)] overflow-y-auto overscroll-y-contain py-1">
            {paletteItems.map((item) => (
              <li key={`${item.kind}-${item.href}-${item.label}`}>
                <PaletteLink
                  href={item.href}
                  className={`group flex min-h-11 items-center justify-between gap-3 px-3 py-2 text-sm hover:bg-foreground/5 ${focusRing}`}
                  onClick={close}
                >
                  ...
                  <span className="text-[0.65rem] uppercase tracking-wider text-foreground/75">
                    {item.kind}
                  </span>
                </PaletteLink>
              </li>
